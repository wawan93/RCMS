...
class RCMS_Database_Driver_MySQLi extends RCMS_Database_Driver {
    ...
    public function connect($host, $username, $password, $name, $charset = 'utf8') {
        if (!$this->db = mysqli_connect($host, $username, $password, $name))
    ...
    public function result() {
        if (mysqli_query($this->db, $this->getSql()))
            return true;
        else
            return false;
    }
    ...
    public function result_num() {
        $num = mysqli_num_rows(mysqli_query($this->db, $this->getSql()));
    ...
    public function result_array() {
        if ($query = mysqli_query($this->db, $this->getSql())) {
            $result = array ();
